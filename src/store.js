...
export default new Vuex.Store({
  ...
  actions: {
    ...
    autoLogin({ commit }) {
      ...
      // Check if the time now is more than set expirationDat set
      if (now >= expirationDate) {
        // if the time is >= to the expirationDate set then return from the funcation
        return;
      }
    ...
    updateUser({ state }) {
      ...
      globalAxios
        .patch(
          "https://chup0007-final.firebaseio.com/users/" +
            state.user.id +
            ".json" +
            "?auth=" +
            state.idToken,
